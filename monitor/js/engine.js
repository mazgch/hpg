...
const mapMsg = {
    ...
    0x0A/*MON*/: { 0x02:'IO',        0x04:'VER',       0x06:'MSGPP',     0x07:'RXBUF',     0x08:'TXBUF',
                   ...
                   0x2E:'SMGR',      0x32:'BATCH',     0x35:'PMP',       0x99:'EVKMEAS'},
...
function process(data, type) {
    ...
    let msgSpec;
    let payload;
    if (m = message.data.match(/\$(G[ABNLPIQ])([A-Z]{3}),(.*)\*[0-9A-F]{2}\r\n$/)) { // Standard Nav device
        message.talker = m[1];
        message.id = m[2];
        payload = m[3];
        ...
        message.name = message.id;
        msgSpec = spec[message.id];
    } else if (m = message.data.match(/\$([A-OQ-Z][A-Z])(G[ABNLPIQ]Q),(.*)\*[0-9A-F]{2}\r\n$/)) { // Poll request to nav device
        message.talker = m[1];
        message.id = m[2];
        payload = m[3];
        message.name = message.talker + message.id;
