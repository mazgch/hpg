...
3. Attach the two LTE antennas to the SMA ports located on the long edges. In order to avoid mechanical stress on the center pin, make sure you do not rotate the antenna itself but only the nut of the SMA connector. 
...
8. After the device is connected you car try to connect to it using its name hpg-XXXXXX. The IP address of the device is also reported reported in the debug output on the console output CDC port at 115'200 baud. A third option is to use the device discovery of the [HPG Monitor](http://hpg.mazg.ch) Some networks do not allow communication between attached wifi clients so it may be better to connect the device to your computer or mobile in hotspot mode. 
9. After you have sucessfully connected you device you need to configure the PointPerfect location device token that you can get from the thingstream portal and set the possible correction sources to use. You can also set the SIM pin but it is often easier to remove the SIM pin from your SIM card with a mobile phone first. 
...
| [PointPerfect](https://www.u-blox.com/en/product/pointperfect) | A continetal wide SSR correction service with coverage in Europe and North America. |
| [AssistNow](https://www.u-blox.com/en/product/assistnow) | A assited GNSS services that allows to speed up the system start. |

The hardware allows different SKUs and can be assembled with alternative modules with compatibe form-factors such as [SARA-R510S](https://www.u-blox.com/en/product/sara-r5-series), [LENA-R8001](https://www.u-blox.com/en/product/lena-r8-series), [ZED-F9P](https://www.u-blox.com/en/product/zed-f9p-module) or [NEO-D9C](https://www.u-blox.com/en/product/neo-d9c-series).
